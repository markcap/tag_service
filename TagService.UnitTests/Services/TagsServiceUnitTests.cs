...
namespace TagService.UnitTests.Services
{
    public class TagsServiceUnitTests
    {
        ...
        public TagsServiceUnitTests(ITestOutputHelper output)
        {
            _output = output;
            _dbContext = DbContextMocker.GetTagDbContext(nameof(TagsServiceUnitTests));
            var mockRepositoryLogger = new Mock<ILogger<TagsRepository>>();
            var mockServicesLogger = new Mock<ILogger<TagsService>>();

            _tagsRepository = new TagsRepository(mockRepositoryLogger.Object, _dbContext);
            _tagsService = new TagsService(mockServicesLogger.Object, _tagsRepository);
        }
        ...
        public async Task GetById_ReturnList(TagAssetType tagAssetType)
        {
            ...
            if (featureIds.Count() > 0)
            {
                ...
                // Act
                var topics = await _tagsService.GetByForeignIdAsync(featureId, tagAssetType, TagContext.topics);
        ...
        public async Task GetById_ReturnEmpty(TagAssetType tagAssetType)
        {
            ...
            // Act
            var featureList = await _tagsService.GetByForeignIdAsync(featureId, tagAssetType, TagContext.topics);
        ...
        public async Task GetByIds_ReturnList(TagAssetType tagAssetType)
        {
            ...
            // Act
            var featureList = await _tagsService.GetByForeignIdsAsync(featureIds, tagAssetType, TagContext.topics);
        ...
        public async Task GetByIds_ReturnEmpty(TagAssetType tagAssetType, TagContext tagContext)
        {
            ...
            if (featureIds.Count() > 0)
            {
                featureList = await _tagsService.GetByForeignIdsAsync(featureIds, altTagAssetType, tagContext);
            }
            else
            {
                featureList = await _tagsService.GetByForeignIdsAsync(new long[] { 99 }.ToList<long>(), altTagAssetType, tagContext);
            }
